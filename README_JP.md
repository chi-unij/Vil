...
『VILLIEN』は、水墨画を意識した和風世界の探索、酒場運営、攻撃予兆を見極めて反撃するBoss Battleを組み合わせた3Dアクション作品です。DirectX 12によるレンダリング基盤からゲームロジック、シェーダー、UI、開発ツールまで、プログラム部分を個人で制作しました。
...
Overworldの「水鏡亭」では一人称視点の酒場運営を行います。注文、注ぎ、配膳、調理、食器回収、洗浄を連続した操作として実装し、売上を使った備品・客数・店舗拡張と、ゲーム内でのテーブル配置にも対応しています。

## 制作情報

| 項目 | 内容 |
|---|---|
| 制作期間 | 2026年4月～2026年9月 |
| 制作人数 | 1名 |
...
- Overworldと酒場を往復できる一人称視点のサービスループ
- 酒・料理の個別注文、調理、食器回収、洗浄、店舗Upgrade
- Imported NPCのSkeletal Animation再利用とinstanceごとの独立pose
...
生成AIは、企画とタスクの整理、設計案の比較、C++／HLSL実装の補助、デバッグ時の原因仮説、テスト項目および提出資料の整理に活用しました。生成された提案をそのまま採用するのではなく、DirectX 12の構成と既存コードに合わせて内容を確認・修正し、最終的な仕様判断、統合、ビルド、実機動作確認は制作者本人が行っています。

活用の狙いは、個人制作で試行回数を増やしながら、描画機能とゲーム体験の両方を短いサイクルで検証することです。特に、Render Passの整理、Tavernの状態遷移、Animationのinstance分離、submission smoke testの設計では、複数案の比較と回帰確認に利用しました。
...
- `Shift`：走る
- `Space`：水鏡UIを開く
- `E`：酒場への出入り、接客、調理・配膳などの操作
- マウス左クリック：水鏡パズルを操作
- `Esc`：設定画面
- `R`：Boss Battleのリスタート／酒場の営業終了
...
提出用Release buildでは、起動、Player Animation、Boss Battleの反撃処理、酒場の接客ループ、OverworldのDXR Reflectionを確認しています。
